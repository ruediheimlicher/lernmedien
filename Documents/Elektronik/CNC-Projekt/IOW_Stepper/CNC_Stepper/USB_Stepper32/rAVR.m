   ...
   //NSLog(@"tempLinienArray: %@",tempLinienArray);
	...
	else
	{
      if ([KoordinatenTabelle count]==0)
      {
         ...
         NSNumber* KoordinateAX=[NSNumber numberWithFloat:0];
         [WertXFeld setFloatValue:0.0];
         [WertYFeld setFloatValue:0.0];
         [WertXStepper setFloatValue:0.0];
         [WertYStepper setFloatValue:0.0];
         [IndexStepper setIntValue:0];
         NSNumber* KoordinateAY=[NSNumber numberWithFloat:0];
...
- (IBAction)reportStopKnopf:(id)sender
{
   ...
   if ([KoordinatenTabelle count]==0)
   {
	...
	{
		// Seite A
		NSPoint tempStartPunktA= NSMakePoint([[[KoordinatenTabelle objectAtIndex:i]objectForKey:@"ax"]floatValue]*zoomfaktor,[[[KoordinatenTabelle objectAtIndex:i]objectForKey:@"ay"]floatValue]*zoomfaktor);
		...
		NSPoint tempEndPunktA= NSMakePoint([[[KoordinatenTabelle objectAtIndex:i+1]objectForKey:@"ax"]floatValue]*zoomfaktor,[[[KoordinatenTabelle objectAtIndex:i+1]objectForKey:@"ay"]floatValue]*zoomfaktor);
      ...
      // Seite B
      NSPoint tempStartPunktB= NSMakePoint([[[KoordinatenTabelle objectAtIndex:i]objectForKey:@"bx"]floatValue]*zoomfaktor,[[[KoordinatenTabelle objectAtIndex:i]objectForKey:@"by"]floatValue]*zoomfaktor);
		NSString* tempStartPunktBString= NSStringFromPoint(tempStartPunktB);
		
		NSPoint tempEndPunktB= NSMakePoint([[[KoordinatenTabelle objectAtIndex:i+1]objectForKey:@"bx"]floatValue]*zoomfaktor,[[[KoordinatenTabelle objectAtIndex:i+1]objectForKey:@"by"]floatValue]*zoomfaktor);
...
*/
   ...
   NSDictionary* tempDic = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:oldax+deltax], @"ax",[NSNumber numberWithFloat:olday + deltay], @"ay",[NSNumber numberWithFloat:oldbx + deltax],@"bx",[NSNumber numberWithFloat:oldby + deltay],@"by",[NSNumber numberWithInt:klickIndex],@"index",NULL];
...
- (IBAction)reportBlockkonfigurieren:(id)sender
{
   ...
   if ([KoordinatenTabelle count])
   {
     ...
     index++;
      ...
      // Hochfahren auf Einlauf. Liegt auf gleicher Hoehe
...
- (IBAction)reportNeuTaste:(id)sender
{
  ...
  // NSLog(@"reportNeuTaste KoordinatenTabelle vor: %@",[KoordinatenTabelle description]);
   ...
   /* 
   if ([CNCTable numberOfRows]&&[CNCTable numberOfSelectedRows]) 
   { 
      NSLog(@"[CNCTable numberOfRows]: %d [CNCTable numberOfSelectedRows]: %d: ",[CNCTable numberOfRows],[CNCTable numberOfSelectedRows]);
   [CNCTable deselectAll:NULL]; 
